...
def reset_dynamicarea_counter(vecID):
    if vecID_dynamicarea_counter_dictionary.get(vecID)[0] != AREA_0 or vecID_dynamicarea_counter_dictionary.get(vecID)[1] != 0:
        print("[INFO]: called reset_dynamicarea_counter")
        vecID_dynamicarea_counter_dictionary[vecID] = (AREA_0, 0)
...
def search_random_edge(laneID, vecID, curr_lane_index, step):
    ...
    if num_links >= 1:
        ...
        if len(list_available_edgs) > 0:
            ...
            while vecID_dynamicarea_counter_dictionary.get(vecID)[1] < 3 and exit_while is False:
                ...
                print("     [UPDATE] currLato, currCountIncrement:", vecID_dynamicarea_counter_dictionary.get(vecID))
...
def update_vec_searchtime_dictionary(vecID, counter, started):
    ...
    print("     [UPDATE] tupla:", vec_searchtime_dictionary[vecID])
...
def reset_vec_searchtime_dictionary(vecID):
    if vec_searchtime_dictionary.get(vecID)[0] != 0 or vec_searchtime_dictionary.get(vecID)[1] is not False:
        print("reset_vec_searchtime_dictionary")
        vec_searchtime_dictionary[vecID] = (0, False)


def check_searchtime_counter(vecID):
    started = vec_searchtime_dictionary.get(vecID)[1]
    if started is True:
        count = vec_searchtime_dictionary.get(vecID)[0]
        if count < 10:
            if vecID_dynamicarea_counter_dictionary.get(vecID)[1] < 3:
                vecID_dynamicarea_counter_dictionary[vecID] = (vecID_dynamicarea_counter_dictionary.get(vecID)[0] * 2, vecID_dynamicarea_counter_dictionary.get(vecID)[1] + 1)
                update_vec_searchtime_dictionary(vecID, 0, True)
                print("     *** Raddoppio la superficie perchè sono passati 10 steps ***")
                print("     currLato:", vecID_dynamicarea_counter_dictionary.get(vecID)[0])
            else:
                destination_edgeXML = get_destinationXML(vecID)
                print("     Il veicolo ritornerà alla sua destinazione iniziale: " + str(destination_edgeXML) + " perchè sono passati 10 step ed ha incrementato al massimo")
        count = vec_searchtime_dictionary.get(vecID)[0] + 1
        update_vec_searchtime_dictionary(vecID, count, True)"""

"""def check_limits_time_area(vecID):
    count_time = vec_searchtime_dictionary.get(vecID)[0]
    counter_increment_area = vecID_dynamicarea_counter_dictionary.get(vecID)[1]
    if count_time >= 10 and counter_increment_area >= 3:
        return True
    return False"""
            ...
            if parkingID is not None:
                ...
                if check_parkingaviability(parkingID):
                    try:
                        ...
                        search_random_edge(curr_last_laneID, vecID, curr_lane_index, step)
                else:
            ...
            if parkingID is not None:
                ...
                if check_parkingaviability(parkingID):
                    try:
                        ...
                        print("     Non è riuscito a parcheggiare a causa di un'eccezione")
                        search_random_edge(curr_laneID, vecID, curr_lane_index, step)
                else:
                    print("     Non è riuscito a parcheggiare a causa della disponibilità")
                    search_random_edge(curr_laneID, vecID, curr_lane_index, step)
            else:
...
def run():
    load_dynamicarea_counter(AREA_0)
    ...
    while traci.simulation.getMinExpectedNumber() != 0:
        ...
        for vecID in currVehiclesList:
            ...
            # print("     tupla(count, started):", vec_searchtime_dictionary.get(vecID))
