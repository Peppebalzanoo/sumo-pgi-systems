...
def update_dynamicarea_counter(vecID, area, counter):
    if vecID_dynamicarea_counter_dictionary.get(vecID)[0] != area or vecID_dynamicarea_counter_dictionary.get(vecID)[1] != counter:
        print(vecID_dynamicarea_counter_dictionary.get(vecID))
...
def getCoordinateCartesianeEdges(destEdgeID, curr_lane_index):
    list_indexes = get_indexesXML_form_edgeID(destEdgeID)
    if curr_lane_index != 0 and curr_lane_index in list_indexes:
        ...
        tuplaXY = traci.simulation.convert2D(destEdgeID, 0.0, 0, False)
    return tuplaXY
...
def search_random_edge_for_parking(laneID, vecID, curr_lane_index, is_critical, curr_laneID):
    ...
    if num_links >= 1:
        if is_critical is True:
            ...
            # C'è almeno una strada collegata a quella corrente
            list_available_edgs = get_available_edges(list_tuple_links, vecID, curr_lane_index, laneID)
        ...
        if parkingID is not None:
            ...
            if check_parkingaviability(parkingID):
                try:
                    ...
                    search_random_edge_for_parking(curr_last_laneID, vecID, curr_lane_index, True, curr_laneID)
            else:
        ...
        if parkingID is not None:
            ...
            if check_parkingaviability(parkingID):
                try:
                    ...
                    print("     Non è riuscito a parcheggiare a causa di un'eccezione")
                    search_random_edge_for_parking(curr_laneID, vecID, curr_lane_index, False, None)
            else:
                print("     Non è riuscito a parcheggiare a causa della disponibilità")
                search_random_edge_for_parking(curr_laneID, vecID, curr_lane_index, False, None)
        else:
    ...
    while traci.simulation.getMinExpectedNumber() != 0:
        ...
        for vecID in currVehiclesList:
            ...
            if traci.vehicle.isStoppedParking(vecID) is False:
                ...
                # space_on_next_step = (1 / 2 * abs(currAcc) * 1) + (currSpeed * 1)
                space_on_next_step = traci.vehicle.getFollowSpeed(vecID, currSpeed, min_gap, leader_speed, max_decel)


                print("    # currSpeed:", currSpeed," , currAcc:", currAcc)
                print("    # currEdge:", currEdgeID, ", lastEdge:", currLastEdgeID)
                print("    # currLane:", currLaneID, ", LaneIndex:", currLaneIndex)
                print("    # distance_to_last:", distanceToLastEdge)
                print("    # space_on_next_step:", space_on_next_step)


                # ___ CRITICAL ROUTINE ___ #
                if space_on_next_step >= distanceToLastEdge and currEdgeID != currLastEdgeID:
                    list_indexes = get_indexesXML_form_edgeID(currLastEdgeID)
                    if currLaneIndex != 0 and currLaneIndex in list_indexes:
                        currLastLaneID = get_laneXML_form_edgeID_index(currLastEdgeID, currLaneIndex)
                    else:
                        currLastLaneID = get_laneXML_form_edgeID_index(currLastEdgeID, 0)
                    critical_routine(vecID, set_vec_parking_destinationXML, currEdgeID, currLastEdgeID, currLastLaneID, currLaneIndex, currLaneID)

                # ___ NORMAL ROUTINE ___ #
                elif space_on_next_step >= distanceToLastEdge and currEdgeID == currLastEdgeID:
