...
def get_available_edges(vecID, curr_laneID, expected_index, last_laneID_excpected, list_tuple_links):
    ...
    for i in range(0, len(list_tuple_links)):
        ...
        if temp_lane != curr_laneID and temp_lane != last_laneID_excpected and temp_lane != traci.vehicle.getLaneID(vecID):
...
def search_random_edge_for_parking(vecID, curr_laneID, expected_index, last_laneID_excpected, space):
    ...
    if num_links >= 1:
        ...
        while vecID_to_dynamicarea_counter_dictionary.get(vecID)[1] < 3 and exit_cond is False:
            ...
            if len(list_available_edgs) > 0:
                ...
                last_edge = traci.lane.getEdgeID(last_laneID_excpected)

                distance_to_random_edge = get_distance_to_last(last_edge, random_edgID, 0.0)

                space_on_next_next_step = traci.vehicle.getFollowSpeed(vecID, space, distance_to_random_edge, traci.lane.getMaxSpeed(last_laneID_excpected), traci.vehicle.getDecel(vecID))
                if space_on_next_next_step >= distance_to_random_edge:
                    ...
                    if num_links_next >= 1:
                        ...
                        if len(aviable_edges) >= 1:
                            random_next_edgID = random.choice(aviable_edges)
                            traci.vehicle.changeTarget(vecID, random_next_edgID)
                            print("     ### Il veicolo ha scelto casualmente " + random_next_edgID)
                            exit_cond = True
                        else:
...
def critical_routine(vecID, curr_edgeID, curr_laneID, curr_lane_index, last_edgeID, last_laneID_excpected, expected_index, space):
        ...
        if parkingID is not None:
            ...
            if check_parking_aviability(parkingID):
                try:
                    ...
                    search_random_edge_for_parking(vecID, curr_laneID, expected_index, last_laneID_excpected, space)
            else:
        ...
        for vecID in curr_vehicles_List:
            ...
            if traci.vehicle.isStoppedParking(vecID) is False:
                ...
                if space_on_next_step >= distance_to_lastedge:
                    ...
                    critical_routine(vecID, curr_edgeID, curr_laneID, curr_lane_index, last_edgeID, last_laneID_excpected, next_expected_index, space_on_next_step)
