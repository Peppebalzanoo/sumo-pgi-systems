...
def set_route_to_parking_edge(vecID, curr_edgeID, last_edgeID):
    ...
    fln = open("log_strategia3.txt", "a")
    print("[INFO set_route()]: Il veicolo", vecID, "SI STA DIRIGENDO VERSO IL PROSSIMO PIÙ GRANDE A EDGE_PARKING:", edge_parking,", LANE_PARKING:", lane_parking, ", PARKING:", parkingID, file=fln)
    fln.close()
...
def main():
    ...
    # sumoCmd = [sumoBinary, "-c", "./strategia3_config/san_francisco_strategia3_75%.sumocfg", "--start"]
    # traci.start(sumoCmd)
    # run("strategia3", "75%")
    # traci.close()
